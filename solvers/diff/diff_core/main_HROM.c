
#include "core.h"

#include "set_modes.h"

#include "set_D_bc.h"
#include "lpod_core.h"
#include "lpod_core_bcsr.h"

#include "HR.h"
#include "DDHR.h"
#include "DDHR2.h"
#include "DDHR_para.h"
#include "DDHR_para_lb.h"
#include "set_matvec.h"
#include "monowrap.h"

#include "main_FOM.h"

const int BUFFER_SIZE = 10000;

static const char* INPUT_FILENAME_COND          = "cond.dat";
static const char* OUTPUT_FILENAME_VTK          = "result_%06d.vtk";
static const char* OUTPUT_FILENAME_ASCII_TEMP   = "temparature_%06d.dat";
static const char* OUTPUT_FILENAME_ASCII_SOURCE = "source_%06d.dat";

void HROM_set_ansvec(
		VALUES*         vals,
	    HLPOD_HR*     	hlpod_hr,
		const int       total_num_nodes)
{
	for(int i = 0; i < total_num_nodes; i++){
		hlpod_hr->HR_T[i] = vals->T[i];
	}
}

void HROM_set_ansvec_para(
		VALUES*         vals,
		HLPOD_DDHR*     hlpod_ddhr,
		const int       total_num_nodes)
{
	for(int i = 0; i < total_num_nodes; i++){
		hlpod_ddhr->HR_T[i] = vals->T[i];
	}
}

/*for Hyper-reduction*/
void HR_output_result_file_vtk(
		BBFE_DATA*     fe,
		VALUES*        vals,
		POD_VALUES*    pod_vals,
		HLPOD_HR*      hlpod_hr,		
		const char*    filename,
		const char*    directory,
		double         t)
{
	FILE* fp;
	fp = BBFE_sys_hlpod_write_fopen(fp, filename, directory);

	switch( fe->local_num_nodes ) {
		case 4:
			BBFE_sys_write_vtk_shape(fp, fe, TYPE_VTK_TETRA);
			break;

		case 8:
			BBFE_sys_write_vtk_shape(fp, fe, TYPE_VTK_HEXAHEDRON);
			break;
	}

	fprintf(fp, "POINT_DATA %d\n", fe->total_num_nodes);
	BB_vtk_write_point_vals_scalar(fp, vals->T, fe->total_num_nodes, "fem-temperature");
	BB_vtk_write_point_vals_scalar(fp, pod_vals->POD_T, fe->total_num_nodes, "pod-temperature");
	BB_vtk_write_point_vals_scalar(fp, hlpod_hr->HR_T, fe->total_num_nodes, "hr-temperature");
	// for manufactured solution
	BBFE_manusol_calc_nodal_error_scalar(
			fe, vals->error, vals->T, hlpod_hr->HR_T);
	BB_vtk_write_point_vals_scalar(fp, vals->error   , fe->total_num_nodes, "hr-fem_abs_error");
	BBFE_manusol_calc_nodal_error_scalar(
			fe, vals->error, pod_vals->POD_T, hlpod_hr->HR_T);
	BB_vtk_write_point_vals_scalar(fp, vals->error   , fe->total_num_nodes, "hr-pod_abs_error");

	double* source;
	source = BB_std_calloc_1d_double(source, fe->total_num_nodes);
	manusol_set_source(fe, source, t);
	BB_vtk_write_point_vals_scalar(fp, source, fe->total_num_nodes, "source");
	BB_std_free_1d_double(source, fe->total_num_nodes);

	fclose(fp);

}
/********/

/*for Hyper-reduction*/
void HR_output_result_file_vtk_para(
		BBFE_DATA*     fe,
		VALUES*        vals,
		POD_VALUES*    pod_vals,
		HLPOD_DDHR*     hlpod_ddhr,		
		const char*    filename,
		const char*    directory,
		double         t)
{
	FILE* fp;
	fp = BBFE_sys_hlpod_write_fopen(fp, filename, directory);

	switch( fe->local_num_nodes ) {
		case 4:
			BBFE_sys_write_vtk_shape(fp, fe, TYPE_VTK_TETRA);
			break;

		case 8:
			BBFE_sys_write_vtk_shape(fp, fe, TYPE_VTK_HEXAHEDRON);
			break;
	}

	fprintf(fp, "POINT_DATA %d\n", fe->total_num_nodes);
	BB_vtk_write_point_vals_scalar(fp, vals->T, fe->total_num_nodes, "fem-temperature");
	BB_vtk_write_point_vals_scalar(fp, pod_vals->POD_T, fe->total_num_nodes, "pod-temperature");
	BB_vtk_write_point_vals_scalar(fp, hlpod_ddhr->HR_T, fe->total_num_nodes, "hr-temperature");
	// for manufactured solution
	BBFE_manusol_calc_nodal_error_scalar(
			fe, vals->error, vals->T, hlpod_ddhr->HR_T);
	BB_vtk_write_point_vals_scalar(fp, vals->error   , fe->total_num_nodes, "hr-fem_abs_error");
	BBFE_manusol_calc_nodal_error_scalar(
			fe, vals->error, pod_vals->POD_T, hlpod_ddhr->HR_T);
	BB_vtk_write_point_vals_scalar(fp, vals->error   , fe->total_num_nodes, "hr-pod_abs_error");

	double* source;
	source = BB_std_calloc_1d_double(source, fe->total_num_nodes);
	manusol_set_source(fe, source, t);
	BB_vtk_write_point_vals_scalar(fp, source, fe->total_num_nodes, "source");
	BB_std_free_1d_double(source, fe->total_num_nodes);

	fclose(fp);

}
/********/



/*for Hyper-reduction*/
void HR_output_files(
		FE_SYSTEM* sys,
		int file_num,
		double t)
{
	const char* filename;
	char fname_vtk[BUFFER_SIZE];
	char fname_tem[BUFFER_SIZE];
	char fname_sou[BUFFER_SIZE];
	snprintf(fname_vtk, BUFFER_SIZE, OUTPUT_FILENAME_VTK, file_num);
	snprintf(fname_tem, BUFFER_SIZE, OUTPUT_FILENAME_ASCII_TEMP, file_num);
	snprintf(fname_sou, BUFFER_SIZE, OUTPUT_FILENAME_ASCII_SOURCE, file_num);
/*
	filename = monolis_get_global_output_file_name(MONOLIS_DEFAULT_TOP_DIR, "./", fname_vtk);

	if(monolis_mpi_get_global_comm_size() == 1 &&sys->pod_vals.num_2nd_subdomains == 1){
        HR_output_result_file_vtk(
                &(sys->fe),
                &(sys->vals),
                &(sys->pod_vals),
                &(sys->hlpod_hr),
                filename,
                sys->cond.directory,
                t);
    }
    else{  
        HR_output_result_file_vtk_para(
                &(sys->fe),
                &(sys->vals),
                &(sys->pod_vals),
                &(sys->hlpod_ddhr),
                filename,
                sys->cond.directory,
                t);
    }

	filename = monolis_get_global_output_file_name(MONOLIS_DEFAULT_TOP_DIR, "./", fname_tem);
	BBFE_write_ascii_nodal_vals_scalar(
			&(sys->fe),
			sys->pod_vals.POD_T,
			filename,
			sys->cond.directory);
*/

	/**** for manufactured solution ****/
/*
	double* source;
	source = BB_std_calloc_1d_double(source, sys->fe.total_num_nodes);
	manusol_set_source(&(sys->fe), source, t);

	filename = monolis_get_global_output_file_name(MONOLIS_DEFAULT_TOP_DIR, "./", fname_sou);
	BBFE_write_ascii_nodal_vals_scalar(
			&(sys->fe),
			source,
			filename,
			sys->cond.directory);
*/
	double L2_error_fem_hr;
	double L2_error_pod_hr;
	if(monolis_mpi_get_global_comm_size() == 1){
		if(sys->pod_vals.num_2nd_subdomains==1){
			L2_error_fem_hr = POD_elemmat_equivval_relative_L2_error_scalar(
					&(sys->fe),
					&(sys->basis),
					&(sys->monolis_com),
					t,
					sys->hlpod_hr.HR_T,
					sys->vals.T);
					
			L2_error_pod_hr = POD_elemmat_equivval_relative_L2_error_scalar(
					&(sys->fe),
					&(sys->basis),
					&(sys->monolis_com),
					t,
					sys->pod_vals.POD_T,
					sys->hlpod_hr.HR_T);
		}
		else{
			L2_error_fem_hr = POD_elemmat_equivval_relative_L2_error_scalar(
					&(sys->fe),
					&(sys->basis),
					&(sys->monolis_com),
					t,
					sys->vals.T,
					sys->hlpod_ddhr.HR_T);
					
			L2_error_pod_hr = POD_elemmat_equivval_relative_L2_error_scalar(
					&(sys->fe),
					&(sys->basis),
					&(sys->monolis_com),
					t,
					sys->pod_vals.POD_T,
					sys->hlpod_ddhr.HR_T);
		}
	}
	else{
		L2_error_fem_hr = POD_elemmat_equivval_relative_L2_error_scalar(
				&(sys->fe),
				&(sys->basis),
				&(sys->monolis_com),
				t,
				sys->vals.T,
				sys->hlpod_ddhr.HR_T);
				
		L2_error_pod_hr = POD_elemmat_equivval_relative_L2_error_scalar(
				&(sys->fe),
				&(sys->basis),
				&(sys->monolis_com),
				t,
				sys->pod_vals.POD_T,
				sys->hlpod_ddhr.HR_T);
	}

	printf("%s L2 error fem-hrom: %e\n", CODENAME, L2_error_fem_hr);

	if(monolis_mpi_get_global_my_rank() == 0){
		FILE* fp;
		fp = BBFE_sys_hlpod_write_add_fopen(fp, "l2_error_fem_hrom.txt", sys->cond.directory);
		fprintf(fp, "%e %e\n", t, L2_error_fem_hr);
		fclose(fp);
	}

	printf("%s L2 error rom-hrom: %e\n", CODENAME, L2_error_pod_hr);

	if(monolis_mpi_get_global_my_rank() == 0){
		FILE* fp;
		fp = BBFE_sys_hlpod_write_add_fopen(fp, "l2_error_rom_hrom.txt", sys->cond.directory);
		fprintf(fp, "%e %e\n", t, L2_error_pod_hr);
		fclose(fp);
	}

	//BB_std_free_1d_double(source, sys->fe.total_num_nodes);
	/***********************************/
}
/********/

void HROM_pre(
		FE_SYSTEM* sys,
		const int num_modes,
		const int num_snapshot,
		const int num_2nd_subdomains)
{
	if(monolis_mpi_get_global_comm_size() == 1){
		if(num_2nd_subdomains == 1){
			monolis_initialize(&(sys->monolis_hr0));
			monolis_initialize(&(sys->monolis_hr));

			hr_get_selected_elements(
				&(sys->fe),
				&(sys->bc),
				sys->fe.total_num_elems,
				sys->pod_vals.num_snapshot,
				sys->pod_vals.num_modes,
				10000,
				1.0e-6,
				&(sys->hlpod_hr),
				sys->cond.directory);
            
            hr_set_selected_elems(
				&(sys->fe),
				&(sys->hlpod_hr),
				sys->fe.total_num_nodes,
				sys->cond.directory);

			lpod_pad_get_nonzero_pattern(
				&(sys->monolis_hr0),
				&(sys->lpod_mat),
				&(sys->lpod_prm),
				sys->pod_vals.num_modes);

			hr_set_reduced_mat(
				&(sys->monolis_hr0),
				&(sys->fe),
				&(sys->basis),
				&(sys->bc),
				&(sys->pod_mat),
				&(sys->hlpod_hr),
				sys->pod_vals.num_modes,
				sys->vals.dt);

			hr_monolis_set_matrix(
				&(sys->monolis_hr0),
				&(sys->hlpod_hr),
				sys->pod_vals.num_modes);

		}
		else{
			monolis_initialize(&(sys->monolis_hr0));
			monolis_initialize(&(sys->monolis_hr));

			ddhr_get_selected_elements2(
				&(sys->fe),
				&(sys->bc),
				sys->fe.total_num_elems,
				sys->pod_vals.num_snapshot,
				sys->pod_vals.pre_num_modes,
				num_2nd_subdomains,
				10000,
				1.0e-6,
				&(sys->hlpod_ddhr),
				sys->cond.directory);

			ddhr_set_selected_elems(
				&(sys->fe),
				&(sys->hlpod_ddhr),
				sys->fe.total_num_nodes,
				num_2nd_subdomains,
				sys->cond.directory);

			ddhr_set_reduced_mat2(
				&(sys->monolis_hr0),
				&(sys->fe),
				&(sys->basis),
				&(sys->bc),
				&(sys->pod_mat),
				&(sys->hlpod_ddhr),
				sys->pod_vals.pre_num_modes,
				num_2nd_subdomains,
				sys->vals.dt);

			ddhr_monolis_set_matrix(
				&(sys->monolis_hr0),
				&(sys->hlpod_ddhr),
				sys->pod_vals.pre_num_modes,
				num_2nd_subdomains);
		}
	}
}


void HROM_pre_online_nonpara(
		FE_SYSTEM* sys,
		const int num_modes,
		const int num_snapshot,
		const int num_2nd_subdomains)
{
	if(monolis_mpi_get_global_comm_size() == 1){
		if(num_2nd_subdomains == 1){
			monolis_initialize(&(sys->monolis_hr0));
			monolis_initialize(&(sys->monolis_hr));

			hr_get_selected_elements(
				&(sys->fe),
				&(sys->bc),
				sys->fe.total_num_elems,
				sys->pod_vals.num_snapshot,
				sys->pod_vals.num_modes,
				10000,
				1.0e-6,
				&(sys->hlpod_hr),
				sys->cond.directory);
            
            hr_set_selected_elems(
				&(sys->fe),
				&(sys->hlpod_hr),
				sys->fe.total_num_nodes,
				sys->cond.directory);
			
			lpod_pad_get_nonzero_pattern(
				&(sys->monolis_hr0),
				&(sys->lpod_mat),
				&(sys->lpod_prm),
				sys->pod_vals.num_modes);

			hr_set_reduced_mat(
				&(sys->monolis_hr0),
				&(sys->fe),
				&(sys->basis),
				&(sys->bc),
				&(sys->pod_mat),
				&(sys->hlpod_hr),
				sys->pod_vals.num_modes,
				sys->vals.dt);

			hr_monolis_set_matrix(
				&(sys->monolis_hr0),
				&(sys->hlpod_hr),
				sys->pod_vals.num_modes);

		}
		else{
			monolis_initialize(&(sys->monolis_hr0));
			monolis_initialize(&(sys->monolis_hr));

			get_neib_subdomain_id_nonpara(
				&(sys->pod_mat),
				&(sys->hlpod_ddhr),
				num_2nd_subdomains);

			ddhr_lb_read_selected_elements(
				&(sys->hlpod_ddhr),
				num_2nd_subdomains,
				sys->cond.directory);

			ddhr_set_reduced_mat3(
				&(sys->monolis_hr0),
				&(sys->fe),
				&(sys->basis),
				&(sys->bc),
				&(sys->pod_mat),
				&(sys->hlpod_ddhr),
				sys->pod_vals.pre_num_modes,
				num_2nd_subdomains,
				sys->vals.dt);
			
			pod_monolis_set_matrix_bcsr(	//set_nonzero_pattern
				&(sys->monolis_hr0),
				&(sys->pod_mat),
				//sys->pod_vals.num_modes / num_2nd_subdomains,
				sys->pod_vals.pre_num_modes,
				sys->cond.directory);

			ddhr_monolis_set_matrix2(
				&(sys->monolis_hr0),
				&(sys->pod_mat),
				&(sys->hlpod_ddhr),
				sys->pod_vals.pre_num_modes,
				num_2nd_subdomains);
		}
	}
}


void HROM_pre_offline(
		FE_SYSTEM* sys,
		const int num_modes,
		const int num_snapshot,
		const int num_2nd_subdomains)
{
	monolis_initialize(&(sys->monolis_hr0));
	monolis_initialize(&(sys->monolis_hr));

	double t = monolis_get_time_global_sync();
//ROM部分へ移行
/*
	get_meta_neib(
		&(sys->monolis_com_pod_solv),
		&(sys->meta_neib),
		sys->cond.directory);

	ddhr_lb_set_neib(
		&(sys->monolis_com_pod_solv),
		&(sys->fe),
		&(sys->lpod_mat),
		&(sys->hlpod_ddhr),
		&(sys->meta_neib),
		num_2nd_subdomains,
		sys->pod_vals.num_snapshot,
		sys->cond.directory);
*/

	//ddhr_lb_write_selected_elements_para(
	ddhr_lb_write_selected_elements_para_1line(
		&(sys->monolis_com_pod_solv),
		&(sys->fe),
		&(sys->bc),
		&(sys->hlpod_ddhr),
		&(sys->lpod_mat),
		&(sys->meta_neib),
		sys->fe.total_num_elems,
		sys->pod_vals.num_snapshot,
		sys->pod_vals.pre_num_modes,
		num_2nd_subdomains,
		10000,
		1.0e-6,
		sys->cond.directory);
/*
	ddhr_lb_get_selected_elements_internal_overlap(
		&(sys->hlpod_ddhr),
//		sys->pod_vals.num_2nd_subdomains,
		sys->cond.directory);
*/	
}


void HROM_pre_online(
		FE_SYSTEM* sys,
		const int num_modes,
		const int num_snapshot,
		const int num_2nd_subdomains)
{
	monolis_initialize(&(sys->monolis_hr0));
	monolis_initialize(&(sys->monolis_hr));

	double t = monolis_get_time_global_sync();
//ROM部分へ移行

	ddhr_lb_read_selected_elements_para(
		num_2nd_subdomains,
		sys->cond.directory);

	ddhr_lb_get_selected_elements_para_add(
		&(sys->hlpod_ddhr),
		sys->pod_vals.num_1st_subdomains,
		sys->cond.directory);

	/*
	ddhr_lb_set_reduced_mat_para(
		&(sys->monolis_hr0),
		&(sys->fe),
		&(sys->basis),
		&(sys->bc),
		&(sys->lpod_mat),
		&(sys->hlpod_ddhr),
		sys->pod_vals.pre_num_modes,
		num_2nd_subdomains,
		sys->vals.dt);

	ddhr_lb_write_reduced_mat(
		sys->hlpod_ddhr.reduced_mat,
		sys->lpod_mat.n_neib_vec,	
		sys->lpod_mat.n_neib_vec,
		sys->cond.directory);
	
	t = monolis_get_time_global_sync();
//	exit(1);
*/
/*
	ddhr_lb_read_reduced_mat(
		sys->hlpod_ddhr.reduced_mat,
		sys->lpod_mat.n_neib_vec,	
		sys->lpod_mat.n_neib_vec,
		sys->cond.directory);
*/


	lpod_hdd_lb_set_hr_podbasis(
		&(sys->monolis_com),
		&(sys->pod_vals),
		&(sys->lpod_mat),
        sys->pod_vals.pre_num_modes);

	ddhr_lb_set_reduced_mat_para_save_memory(
		&(sys->monolis_hr0),
		&(sys->fe),
		&(sys->basis),
		&(sys->bc),
		&(sys->pod_vals),
		&(sys->lpod_mat),
		&(sys->hlpod_ddhr),
		sys->pod_vals.pre_num_modes,
		num_2nd_subdomains,
		sys->vals.dt);

	hlpod_set_comm_nzpattern_bcsr(
		&(sys->monolis_hr0),
		&(sys->monolis_com_pod_solv),
		&(sys->lpod_mat),
		sys->pod_vals.pre_num_modes,
		sys->cond.directory);

	ddhr_hlpod_calc_block_mat_bcsr_pad(
		&(sys->monolis_hr0),
		&(sys->monolis_com_pod_solv),
		&(sys->lpod_com),
		&(sys->lpod_mat),
		&(sys->lpod_prm),
		&(sys->hlpod_ddhr),
		&(sys->meta_neib),
		sys->pod_vals.pre_num_modes,
		sys->pod_vals.num_2nd_subdomains,
		sys->cond.directory);
	
}


void HROM_nonparallel(
    FE_SYSTEM       sys,
    const int       step_HR,
    const int       step_POD,
    const double    t)
{
    double t1 = monolis_get_time();    

    monolis_copy_mat_value_R(&(sys.monolis_hr0), &(sys.monolis_hr));
	monolis_clear_mat_value_rhs_R(&(sys.monolis_hr));

    double set_bc1 = monolis_get_time();
	/*
	hlpod_sys_manusol_set_bc(
			&(sys.fe),
			&(sys.bc),
			BLOCK_SIZE,
			t,
			manusol_get_sol,
			&(sys.lpod_prm));
	*/
	hlpod_hr_sys_manusol_set_bc(
			&(sys.fe),
			&(sys.bc),
			BLOCK_SIZE,
			t,
			manusol_get_sol,
			&(sys.lpod_prm));
	double set_bc2 = monolis_get_time();

    if(sys.pod_vals.num_2nd_subdomains == 1){

        if((step_HR-step_POD) == 1){
            HROM_set_ansvec(
                &(sys.vals),
                &(sys.hlpod_hr),
                sys.fe.total_num_nodes);
        }

        hr_set_reduced_vec(
            &(sys.monolis_hr),
            &(sys.fe),
            &(sys.basis),
            &(sys.hlpod_hr),
            &(sys.pod_mat),
            sys.pod_vals.num_modes,
            sys.vals.dt,
            t);
        
        hr_set_D_bc(
            &(sys.monolis_hr),
            &(sys.fe),
            &(sys.basis),
            &(sys.bc),
            &(sys.pod_mat),
            &(sys.hlpod_hr),
            sys.pod_vals.num_modes,
            sys.vals.dt);

        hr_to_monollis_rhs(
            &(sys.monolis_hr),
            &(sys.hlpod_hr),
            sys.pod_vals.num_modes);
        
        double t2 = monolis_get_time();

        FILE* fp;
        fp = BBFE_sys_hlpod_write_add_fopen(fp, "calctime/hr_time_calc_mat.txt", sys.cond.directory);
        fprintf(fp, "%e %e\n", t, t2-t1 + set_bc2 - set_bc1);
        fclose(fp);

    
        BBFE_sys_monowrap_solve(
            &(sys.monolis_hr),
            &(sys.monolis_com_pod),
            sys.pod_mat.pod_coordinates,
            MONOLIS_ITER_CG,
            MONOLIS_PREC_DIAG,
            sys.vals.mat_max_iter,
            sys.vals.mat_epsilon);

        double t1 = monolis_get_time();

        hr_calc_solution(
            &(sys.fe), 
            &(sys.pod_mat), 
            &(sys.hlpod_hr), 
            &(sys.bc), 
            sys.pod_vals.num_modes,
            &(sys.lpod_prm));

        t2 = monolis_get_time();

        //FILE* fp;
        fp = BBFE_sys_hlpod_write_add_fopen(fp, "calctime/hr_time_calc_sol.txt", sys.cond.directory);
        fprintf(fp, "%e %e\n", t, t2-t1);
        fclose(fp);

    }
    else{

        if((step_HR-step_POD) == 1){
            HROM_set_ansvec_para(
                &(sys.vals),
                &(sys.hlpod_ddhr),
                sys.fe.total_num_nodes);
        }
        double t1 = monolis_get_time();

        ddhr_set_reduced_vec3(
            &(sys.monolis_hr),
            &(sys.fe),
            &(sys.basis),
            &(sys.hlpod_ddhr),
            &(sys.pod_mat),
            sys.pod_vals.pre_num_modes,
            sys.pod_vals.num_2nd_subdomains,
            sys.vals.dt,
            t);
        
        ddhr_set_D_bc3(
            &(sys.monolis_hr),
            &(sys.fe),
            &(sys.basis),
            &(sys.bc),
            &(sys.pod_mat),
            &(sys.hlpod_ddhr),
            sys.pod_vals.pre_num_modes,
            sys.pod_vals.num_2nd_subdomains,
            sys.vals.dt);

        ddhr_to_monollis_rhs(
            &(sys.monolis_hr),
			&(sys.pod_mat),
            &(sys.hlpod_ddhr),
            sys.pod_vals.pre_num_modes,
            sys.pod_vals.num_2nd_subdomains);

        double t2 = monolis_get_time();

        FILE* fp;
        fp = BBFE_sys_hlpod_write_add_fopen(fp, "calctime/hr_time_calc_mat.txt", sys.cond.directory);
        fprintf(fp, "%e %e\n", t, t2-t1);
        fclose(fp);
        
		/*
        BBFE_sys_monowrap_solve(
            &(sys.monolis_hr),
            &(sys.monolis_com_pod),
            sys.pod_mat.pod_coordinates,
            MONOLIS_ITER_CG,
            MONOLIS_PREC_DIAG,
            sys.vals.mat_max_iter,
            sys.vals.mat_epsilon);
		*/
		
		BBFE_sys_monowrap_solve_V(
			&(sys.monolis_hr),
			&(sys.monolis_com_pod),
			sys.pod_mat.pod_coordinates,
			MONOLIS_ITER_CG,
			MONOLIS_PREC_DIAG,
			sys.vals.mat_max_iter,
			sys.vals.mat_epsilon,
			sys.pod_mat.num_modes_internal);

        t1 = monolis_get_time();

        ddhr_calc_solution(
            &(sys.fe), 
            &(sys.pod_mat), 
            &(sys.hlpod_ddhr), 
            &(sys.bc), 
            sys.pod_vals.pre_num_modes,
            sys.pod_vals.num_2nd_subdomains,
			1,
            &(sys.lpod_prm));

        t2 = monolis_get_time();

        //FILE* fp;
        fp = BBFE_sys_hlpod_write_add_fopen(fp, "calctime/hr_time_calc_sol.txt", sys.cond.directory);
        fprintf(fp, "%e %e\n", t, t2-t1);
        fclose(fp);

        output_hr_monolis_solver_prm(&(sys.monolis_hr), sys.cond.directory, t);
    }
}


void HROM_hierarchical_parallel(
    FE_SYSTEM sys,
    const int step_HR,
    const int step_POD,
    const double t)
{
    if((step_HR-step_POD) == 1){
        HROM_set_ansvec_para(
            &(sys.vals),
            &(sys.hlpod_ddhr),
            sys.fe.total_num_nodes);
    }

    double t1 = monolis_get_time();

    //monolis_copy_mat_value_R(&(sys.monolis_hr0), &(sys.monolis_hr));
	monolis_copy_mat_value_matrix_R(&(sys.monolis_hr0), &(sys.monolis_hr));
	monolis_clear_mat_value_rhs_R(&(sys.monolis_hr));

	double set_bc1 = monolis_get_time();
	hlpod_hr_sys_manusol_set_bc(
			&(sys.fe),
			&(sys.bc),
			BLOCK_SIZE,
			t,
			manusol_get_sol,
			&(sys.lpod_prm));
	double set_bc2 = monolis_get_time();
	
    ddhr_lb_set_reduced_vec_para(
        &(sys.monolis_hr),
        &(sys.fe),
        &(sys.basis),
        &(sys.hlpod_ddhr),
        &(sys.lpod_mat),
        sys.pod_vals.pre_num_modes,
        sys.pod_vals.num_2nd_subdomains,
        sys.vals.dt,
        t);
    
    ddhr_lb_set_D_bc_para(
        &(sys.monolis_hr),
        &(sys.fe),
        &(sys.basis),
        &(sys.bc),
        &(sys.lpod_mat),
        &(sys.hlpod_ddhr),
        sys.pod_vals.pre_num_modes,
        sys.pod_vals.num_2nd_subdomains,
        sys.vals.dt);

    ddhr_to_monollis_rhs_para_pad(
        &(sys.monolis_hr),
        &(sys.hlpod_ddhr),
        &(sys.lpod_mat),
		sys.pod_vals.num_2nd_subdomains,
        sys.pod_vals.pre_num_modes);

    double t2 = monolis_get_time();

    if(monolis_mpi_get_global_my_rank() == 0){
        FILE* fp;
        fp = BBFE_sys_hlpod_write_add_fopen(fp, "calctime/hr_time_calc_mat.txt", sys.cond.directory);
        fprintf(fp, "%e %e\n", t, t2-t1);
        fclose(fp);
    }


    BBFE_sys_monowrap_solve(
        &(sys.monolis_hr),
        &(sys.monolis_com_pod_solv),
        sys.lpod_mat.coordinates,
        MONOLIS_ITER_CG,
        MONOLIS_PREC_DIAG,
        sys.vals.mat_max_iter,
        sys.vals.mat_epsilon);

/*
	BBFE_sys_monowrap_solve_V(
		&(sys.monolis_hr),
		&(sys.monolis_com_pod_solv),
		sys.lpod_mat.coordinates,
		MONOLIS_ITER_CG,
		MONOLIS_PREC_DIAG,
		sys.vals.mat_max_iter,
		sys.vals.mat_epsilon,
		sys.lpod_mat.n_dof_list);
*/
    t1 = monolis_get_time();

    lpod_pad_calc_block_solution_local_para_pad(
        &(sys.monolis_com),
        &(sys.fe),
        &(sys.hlpod_ddhr),
        &(sys.lpod_mat),
        &(sys.bc),
        sys.pod_vals.num_2nd_subdomains,
		sys.pod_vals.pre_num_modes,
        &(sys.lpod_prm));

    t2 = monolis_get_time();

    if(monolis_mpi_get_global_my_rank() == 0){
        FILE* fp;
        fp = BBFE_sys_hlpod_write_add_fopen(fp, "calctime/hr_time_calc_sol.txt", sys.cond.directory);
        fprintf(fp, "%e %e\n", t, t2-t1);
        fclose(fp);
    }

	output_hr_monolis_solver_prm(&(sys.monolis_hr), sys.cond.directory, t);

}
